...
async function loadCourses(){
  try{
    ...
    if(termLink)termLink.setAttribute('aria-label',`ดูรายวิชา ${termText} ${yearText}`);
    if(schedulePeriod)schedulePeriod.textContent=data.schedule?.period?`ช่วงเรียน ${data.schedule.period}`:'';
    if(grid)grid.replaceChildren(...data.courses.map(renderCourse));
  }catch(error){
    if(grid){
